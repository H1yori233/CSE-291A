...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--task_config", type=str, default=None, help="Path to a single task config .json file to run")
    return parser
...
def build_env(args: argparse.Namespace) -> DesktopEnv:
    ...
    # If running a single task, snapshot lookup might be tricky if not provided.
    # OSWorld generally expects valid snapshot names.
    return DesktopEnv(
...
def main():
    ...
    try:
        if args.task_config:
            # Mode 1: Run single task from file
            ...
            # Try to infer domain from path or default to 'unknown'
            domain = "unknown"
            if "examples" in config_path:
                try:
                    # e.g. .../examples/vlc/xxx.json -> vlc
                    domain = os.path.basename(os.path.dirname(config_path))
                except Exception:
                    pass
            ...
            logger.info("Running single task: %s/%s", domain, example_id)
            ...
            # Mode 2: Run batch from meta file (original behavior)
